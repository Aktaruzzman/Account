...
class MY_Model extends CI_Model
{
    ...
    public function get_code($type = "basic", $length = 4, $database = true)
    {
        return $this->_generate_code($type, $length, $database);
    }
    private function _generate_code($type, $length = 4, $database = null)
    {
        $newkey = null;
        if ($database) {
            do  $newkey = random_string($type, $length);
            while ($this->_code_exists($newkey));
            return $newkey;
        } else return random_string($type, $length);
    }
    ...
    function isLoggedIn()
    {
        return $this->isOfficerIn();
    }
    function isOfficerIn()
    {
        return $this->session->userdata('officer_id') != false;
    }
    function officerOut()
    {
        return $this->session->unset_userdata('officer_id');
    }
}
